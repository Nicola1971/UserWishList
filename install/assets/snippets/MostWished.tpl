...
try {
    ...
    $tvQuery = $modx->db->select("id", "`devsite_db`.`fzl6_site_tmplvars`", "name = '{$userTv}'");
    if ($tvRow = $modx->db->getRow($tvQuery)) {
        ...
        $query = $modx->db->query("
            ...
            FROM 
                `devsite_db`.`fzl6_user_values` uv
                CROSS JOIN (
        ...
        $params = array('api' => '1', 'documents' => implode(',', array_keys($docCounts)), 'debug' => isset($debug) ? $debug : '0', 'tvPrefix' => isset($tvPrefix) ? $tvPrefix : '', 'tvList' => isset($tvList) ? $tvList : '', 'summary' => isset($summary) ? $summary : 'notags,len:300', 'orderBy' => 'FIELD(c.id, ' . implode(',', array_keys($docCounts)) . ')');
        $output = '';
        ...
        if (is_array($items)) {
            foreach ($items as $item) {
                ...
                $replacements = array('[+id+]' => $item['id'], '[+pagetitle+]' => $item['pagetitle'], '[+title+]' => $item['title'], '[+introtext+]' => $item['introtext'], '[+wishlist_count+]' => $count, '[+summary+]' => $item['summary']);
